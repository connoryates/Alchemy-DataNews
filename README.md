...
Alchemy::DataNews - Perl style queries for IBM Watson DataNews API.
...
This module will map Perl syntax into the REST parameters that Watson's DataNews API uses - similar to how [DBIC](http://search.cpan.org/~ribasushi/DBIx-Class-0.082840/lib/DBIx/Class.pod) and [Search::Elasticsearch](https://metacpan.org/pod/Search::Elasticsearch)  work.
...
```timeframe``` - Specifies a timeframe for your search. The ```start``` key indicates when your query should start looking and
...
Will start the search beginning two days in the past from the current day and end on the current day.

You don't have to specify an amount before/unit of time though, you can specify dates as well:
...
Taxonomies are scored by their ```confidence``` levels. You can filter your taxonomy queries by confidence score:
...
Both `entity` and `action` keys will accept an `ArrayRef` as an argument as well.
...
output type is ```XML``` - which is only available for ```raw_output```.


```next``` - returns the next page of results. If there is a next page of results, the previous result will contain the key "next" with a token as the value that
...
Since the next token is cached within the ```package``` you can check if a next page is available with the following::
...
# The last query run is automatically cached, so passing in undef will
# default to the cached query (which is still cached with raw_ouput)
...
```perl
my $response = $alchemy->fetch_query({
